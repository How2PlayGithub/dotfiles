...
scrDir=${scrDir:-$HOME/.local/share/bin}
...
fn_profile() {
    local profilePath="$HOME/.local/share/icons/Wallbash-Icon/hyde"
    if [ -f "$HOME/.face.icon" ]; then
        cp "$HOME/.face.icon" "${profilePath}.png"
    else
        cp "$HOME/.local/share/icons/Wallbash-Icon/hyde.png" "${profilePath}.png"
    fi
    return 0
}
